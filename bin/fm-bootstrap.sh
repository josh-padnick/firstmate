...
#                 "LINEAR: <activation or actionable setup result>",
#                 "TANGLE: <remediation>",
...
#          secondmate_handoff_resume, x_mode_setup, linear_mode_setup,
...
#          Relay or Linear connector artifacts, project clones, or repair instructions.
...
#                 x_mode_setup or linear_mode_setup: those already ran on the
#                 local pass.
...
connector_cadence_content() {
  cat <<'EOF'
# Auto-generated by fm-bootstrap.sh - connector watcher cadence.
# Source this before the active harness protocol starts a watcher process so
# fm-watch.sh polls active Relay or Linear checks every 30s. Homes without either
# connector have no such file and keep the default 300s cadence.
export FM_CHECK_INTERVAL=30
EOF
}

connector_cadence_publish() {  # <path>
  local cadence=$1 body
  body=$(connector_cadence_content)
  x_mode_write_if_changed "$cadence" "$body" 600
}
...
x_mode_setup() {
  ...
  x_mode_remove_artifacts() {
    ...
    [ "$linear_active" -eq 1 ] || x_mode_remove_artifact "$cadence" || failed=1
    [ "$failed" -eq 0 ]
  }
  ...
  connector_cadence_publish "$cadence" || { fmx_arm_failed; return 0; }
...
linear_mode_setup() {
  ...
  if ! command -v shasum >/dev/null 2>&1 && ! command -v sha256sum >/dev/null 2>&1; then
    echo "MISSING_MANUAL: sha256 (instructions: install shasum or sha256sum)"
    missing=1
  fi
  [ "$missing" -eq 0 ] || {
    echo "LINEAR: poll not armed because required tools are missing"
    return 0
  }

  mkdir -p "$STATE" "$CONFIG" 2>/dev/null || {
    echo "LINEAR: failed to prepare state or config directory"
    return 0
  }
  case "$FM_HOME" in
    /*) shim_home=$FM_HOME ;;
    *)
      shim_home=$(CDPATH='' cd -- "$FM_HOME" 2>/dev/null && pwd -P) || {
        echo "LINEAR: failed to resolve the operational home"
        return 0
      }
      ;;
  esac
  shim_body=$(fm_linear_poll_shim_content "$shim_home" "$FM_ROOT")
  fm_linear_poll_shim_valid "$shim" "$shim_home" "$FM_ROOT" || changed=1
  [ -f "$cadence" ] && [ ! -L "$cadence" ] || changed=1
  x_mode_write_if_changed "$shim" "$shim_body" 700 || {
    echo "LINEAR: failed to publish the poll shim"
    return 0
  }
  fm_linear_poll_shim_valid "$shim" "$shim_home" "$FM_ROOT" || {
    echo "LINEAR: poll shim validation failed"
    return 0
  }
  if ! FM_HOME="$FM_HOME" FM_STATE_OVERRIDE="$STATE" \
    "$SCRIPT_DIR/fm-check-register.sh" fm-linear-inbox >/dev/null 2>&1; then
    echo "LINEAR: failed to register the poll shim"
    return 0
  fi

  connector_cadence_publish "$cadence" || {
    echo "LINEAR: poll armed, but failed to publish the 30s watcher cadence"
    return 0
  }

  for artifact in .linear-absorb .linear-state-snapshot .linear-inbox-seen .linear-comment-cursor; do
    if [ -e "$STATE/$artifact" ] || [ -L "$STATE/$artifact" ]; then
      changed=1
      if ! x_mode_remove_artifact "$STATE/$artifact"; then
        echo "LINEAR: new poll armed, but failed to remove legacy state/$artifact"
        return 0
      fi
    fi
  done
  [ "$changed" -eq 0 ] \
    || echo "LINEAR: event-ledger poll armed via state/fm-linear-inbox.check.sh; legacy absorb state removed; 30s watcher cadence active"
}
...
if [ "${FM_BOOTSTRAP_DETECT_ONLY:-0}" != 1 ]; then
  ...
  # Connector setup writes local Relay or Linear artifacts only and never leaves the machine.
  local_phase && x_mode_setup
  local_phase && linear_mode_setup
