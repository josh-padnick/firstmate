...
| Fact | Value |
|---|---|
| Busy state | The one remaining rendered-tail fallback, isolated to Grok until its structured lifecycle is live-verified; it accepts independently observed active-turn signals - the legacy `Ctrl+c:cancel`, plus the structural spinner status row's `Waiting for response…` and `[stop]` - while never treating the braille spinner alone as busy; [`docs/verification/supervision.md`](../../../docs/verification/supervision.md#semantic-busy-state) owns the current versioned evidence and refresh command. |
