...
| `fm-linear-poll.sh`      | Poll Linear into the durable identity-attributed event ledger                         |
| `fm-linear-act.sh`       | Route state, assignee, and comment writes through the journaled Linear door           |
| `fm-linear-lib.sh`       | Share Linear credentials, identity, transport, timestamp, and private-state primitives |
