...
The Claude mode waits up to `FM_CLAUDE_AUTOARM_SYNC_WAIT_MS` (default 800 milliseconds) and allows the stop when the watcher is healthy, the auto-arm's generation claim is open, or `state/.claude-autoarm-epoch` contains a fresh actionable rewake result.
...
Anything else - a finished outcome including `rewake`, a dead or identity-mismatched owner, a stuck owner, an identityless entry, or no entry - lets the same Stop-owned firing take the next generation and arm; taking a newer generation is the reclaim, and a steady-state predecessor is never signalled or revoked.
No mutex is held across arming or output: `state/.claude-autoarm.lock` survives only as a micro-mutex serializing individual ledger writes.
A claimant retries bare micro-mutex contention for a bounded interval within the same firing and rechecks for an open winning generation between attempts, so concurrent firings converge on exactly one owner while a recoverable stale claim does not wait for another Stop.
Claude exposes no Stop occurrence identity that distinguishes sequential identical payloads, so a duplicate firing delayed until after its peer commits may take the next generation; for one duplicated delivery the bound is one additional restart attempt, which the watcher singleton and healthy-cycle attachment absorb.
A superseded owner goes completely silent - ownership is re-verified before every arm invocation, episode-state mutation, ledger write, and continuation.
