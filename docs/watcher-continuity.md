...
It also covers generation-claim single-flight, same-firing dead-claim supersession after transient mutex contention for every recorded outcome, stuck-claim supersession, superseded-owner silence, notice-marker refusal and retry, ownership-atomic episode reset, and the legacy upgrade shim; [`turnend-guard.md`](turnend-guard.md) owns those behavior contracts.
`FM_CLAUDE_LIVE_E2E=1 tests/fm-claude-stop-autoarm-live-e2e.test.sh` starts with the reproduced stale session lock and stale dead-owner `rewake` generation, runs session start first, proves that generation is superseded, completes two tokenless cycles, and checks the competing-live-owner negative control.
...
[`verification/supervision.md`](verification/supervision.md#watcher-continuity) records the current five-harness live evidence, the 2026-08-27 Stop-owned Claude auto-arm results, and exact opt-in commands.
