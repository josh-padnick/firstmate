...
The blocking and bounded-follow-up mechanisms were validated across six harnesses on 2026-07-08 through 2026-08-27, with Claude's replacement Stop-owned path revalidated on 2026-08-27 and Cursor's stop-hook park validated on 2026-08-13.

| Harness | Version verified | Mechanism | Observed result |
| --- | --- | --- | --- |
| Claude | 2.1.250 | Cooperative blocking `Stop` guard plus `asyncRewake` auto-arm | A fresh unsupervised session ran session start first, reclaimed a stale dead-owner session lock, superseded a stale dead-owner `rewake` generation above epoch 8850, completed two tokenless rewake cycles with no model arm command or guard continuation, and left a competing live owner unchanged. |
...
The Claude product live path ran with Claude Code 2.1.250 on 2026-08-27:
...
```text
2.1.250 (Claude Code)
ok - Claude 2.1.250 (Claude Code) live E2E reclaimed a stale session lock through session start, completed two tokenless Stop-owned rewake cycles, and preserved the competing-live-owner boundary
```
...
The cross-harness evidence combines the 2026-07-17 live pass with Claude's replacement Stop-owned path revalidated on 2026-08-27, all against isolated project and home state.
...
| --- | --- | --- |
| Claude | `FM_CLAUDE_LIVE_E2E=1 tests/fm-claude-stop-autoarm-live-e2e.test.sh` | Session start reclaimed a stale session owner, the first Stop superseded a stale dead-owner `rewake` generation above epoch 8850 before two Stop-owned cycles, and a competing live owner prevented arm, rewake, epoch write, or lock replacement. |
