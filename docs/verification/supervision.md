...
| Grok | 1.0.5 | Isolated rendered-tail fallback | A real turn rendered both `Waiting for response…` and `[stop]`; either independently classified busy and the completed turn classified idle, while the adapter remains unconverted because no structured lifecycle source is verified. |
...
Live drift guard, verified on 2026-08-20 with Grok 1.0.5:
...
```text
ok - Grok (grok 1.0.5 (5115b46bc909) [stable]): live waiting-response and stop signals classify busy, then settle idle
```
