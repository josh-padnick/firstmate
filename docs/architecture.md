...
A positive busy verdict proves that a turn is active and clears the stale escalation ladder.
Idle and unknown verdicts remain fail-safe and take the ordinary stale classification path.
...
A crew that directly declares `paused:` for a known external wait is separately absorbed while idle regardless of pane harness or process liveness and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge.
For an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake, then applies that same cadence to an unchanged durable `captain-held` endpoint only when the backend confidently reports its agent dead.
