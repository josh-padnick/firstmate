...
Two of those classes are paced rather than repeated at pane-hash rate, because the pane hash is a fragile proxy that any fleet-wide re-render invalidates for every pane at once.
A pane parked on a captain-relevant result that firstmate has already delivered re-surfaces only when that status line changes or once per `FM_TERMINAL_RESURFACE_SECS`, and repeat wedge escalations for a stale pane with no positive evidence stretch by `FM_STALE_ESCALATE_UNKNOWN_MULT` while its semantic busy verdict is unknown rather than a proven idle.
Neither paces a first surface, and the heartbeat backstop still recovers anything the per-wake path absorbed by mistake.
...
For an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake, then absorbs an unchanged `paused:` or durable `captain-held` endpoint the same way once the backend confidently reports its agent dead: only the `paused:` case still re-surfaces on the pause cadence for a recheck, while a `captain-held` endpoint - already durable in the captain-held backlog record - never re-surfaces again (Linear BIG-45).
In away mode, the daemon clears wedge and pause-recheck timers for a stale `captain-held` endpoint while retaining its hash suppressor until the status changes, and it preserves the existing bounded recheck cadence for `paused:` external waits.
