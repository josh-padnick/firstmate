...
For an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake, then absorbs an unchanged `paused:` or durable `captain-held` endpoint the same way once the backend confidently reports its agent dead: only the `paused:` case still re-surfaces on the pause cadence for a recheck, while a `captain-held` endpoint - already durable in the captain-held backlog record - never re-surfaces while unchanged.
The [afk classification policy](../.agents/skills/afk/SKILL.md#classification-policy) owns the mode-specific contract for away-mode stale panes; the daemon shares the watcher vocabulary while maintaining its own timer and suppressor lifecycle.
