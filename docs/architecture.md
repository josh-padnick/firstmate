...
In away mode, the daemon also clears wedge and pause-recheck tracking for a stale `captain-held` endpoint while preserving the existing bounded recheck cadence for `paused:` external waits.
