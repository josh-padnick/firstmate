...
The poller reads stable user IDs on comments and issue history, atomically publishes complete events under `state/linear-inbox/`, records each comment's latest hash and `editedAt` in `state/.linear-comment-heads.tsv`, canonical comment roots in `state/.linear-comment-roots.tsv`, each history record's latest derived-content hash in `state/.linear-history-heads.tsv`, durable Firstmate thread participation in `state/.linear-thread-participation.tsv`, and issue snapshots in `state/.linear-issue-heads.json`, and only then advances the server-timestamp cursors in `state/.linear-cursor`.
Incomplete provider traversals are privately checkpointed under `state/.linear-history-scans/`, `state/.linear-thread-root-scans/`, and `state/.linear-thread-descendant-scans/` so a bounded poll resumes from durable progress.
...
`FM_LINEAR_SELF_MENTION` can override the exact mention token used for that routing and defaults to `@josh.padnickfirstmate`.
...
Removing the key before the first arm is the deliberate local opt-out; after an arm, key removal is a credential failure rather than an opt-out because silently stopping observation could lose captain input.
Linear deletion events are deliberately not covered by this polling contract, and moving an issue to `Canceled` means stop work on it.
