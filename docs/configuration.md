...
FM_STALE_ESCALATE_UNKNOWN_MULT=4   # multiplies FM_STALE_ESCALATE_SECS for REPEAT wedge escalations on a stale pane that has no positive evidence and whose semantic busy verdict is unknown rather than a proven idle; never delays that pane's first surface, and never applied to a provably-working pane
...
FM_TERMINAL_RESURFACE_SECS=        # unset/blank defaults to FM_PAUSE_RESURFACE_SECS; seconds before a pane parked on an already-delivered captain-relevant result re-surfaces as a bounded reminder instead of re-firing on every pane-hash change
