...
2. Source `__FM_X_MODE_ENV__` first when Relay or Linear is active.
...
4. Ordinary wake: re-arm the next cycle with the same background `bin/fm-watch-arm.sh` call if work remains in flight or a connector still needs polling.
