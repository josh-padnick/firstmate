...
- **Optional Linear** - opt in with private API and stable user IDs so routed comments and supported board changes enter a durable authority-attributed inbox before polling advances, while every Firstmate state, assignee, and comment write goes through one retry-safe journaled path.
...
Optional secondmates extend this to persistent local or whole-home remote second mates, dispatch profiles let you steer which harness handles which task, opt-in Linear adds a durable board channel, and opt-in Relay lets the same fleet answer public mentions.
`codex-app` is not a runtime backend yet; [docs/codex-app-backend.md](docs/codex-app-backend.md) owns the Codex App boundary.

Full architecture - the supervision engine, worktree isolation, secondmates, dispatch profiles, project modes, optional Linear and Relay, fleet sync, and self-update - is in [docs/architecture.md](docs/architecture.md).
...
- [docs/configuration.md](docs/configuration.md) - environment variables, `FM_HOME`, runtime backend selection, optional Linear and Relay setup, the files you set, and harness support.
