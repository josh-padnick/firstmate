...
# Setup
You are in a disposable git worktree of someapp, at a detached HEAD on a clean default branch.
...
1. First action: create your branch: `git checkout -b fm/demo-ux-local`

# Rules
1. Never push to any remote and never open a PR. Work only on your `fm/demo-ux-local` branch; firstmate handles the merge into local `main`.
2. Stay inside this worktree; modify nothing outside it.
3. Use gh-axi for GitHub operations and chrome-devtools-axi for browser operations.
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '/tmp/fm-ux-e2e.K3BDNX/state/demo-ux-local.status'`
...
# Project memory
If `AGENTS.md` or `CLAUDE.md` already exists, or if this task produced durable project-intrinsic knowledge, run `/Users/personal/.no-mistakes/worktrees/a199dc8e0fb9/01M08G32KDT7JGZMG5R5T2Q7G5/bin/fm-ensure-agents-md.sh .` in the worktree.
Record only project knowledge useful to almost every future session.
For anything the codebase already shows, prefer a pointer to the authoritative file, command, or doc over copying the detail.
If you touch a project `AGENTS.md` that lacks `## Maintaining this file`, add that short self-governance section from `/Users/personal/.no-mistakes/worktrees/a199dc8e0fb9/01M08G32KDT7JGZMG5R5T2Q7G5/bin/fm-ensure-agents-md.sh` in the same pass.
Keep it proportionate: skip `AGENTS.md` edits for trivial tasks that produced no durable project knowledge.

# Definition of done
Delivery contract: mode=local-only
This task ships **local-only**: no remote, no PR, no pipeline.
The task is complete only when committed on your branch `fm/demo-ux-local`. Do NOT push, do NOT open a PR, do NOT merge.
Keep your branch a clean fast-forward onto the current default branch - if `main` has advanced, rebase onto it so the eventual merge stays a fast-forward.
When it is implemented and committed, append `done: ready in branch fm/demo-ux-local` to the status file and stop.
