...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  mission-control.json  captain's mission-control state document (schema fm-mission-control.v1), generated by bin/fm-dashboard.sh
  mission-control.html  the single-viewport page rendered from that document; `--watch` keeps it current
...
state/               volatile runtime signals; gitignored
  ...
  dashboard-prs.json  mission-control's pull-request check cache; written only by fm-dashboard.sh --refresh-prs, and its age is published on the board
