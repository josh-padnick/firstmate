...
.env                 optional Relay and Linear credentials; LOCAL, gitignored; presence-gates their connector setup
...
state/               runtime records and signals; gitignored
  ...
  fm-linear-inbox.check.sh  generated Linear event-ledger poll shim; hash-registered when a configured key first activates the connector
  linear-inbox/ linear-outbox/  durable authority-attributed event inbox and journaled Firstmate write intents; linear-operations owns handling
  .linear-cursor .linear-bootstrap-horizon .linear-comment-heads.tsv .linear-comment-roots.tsv .linear-comment-head-bootstrap.json .linear-history-heads.tsv .linear-thread-participation.tsv .linear-issue-heads.json .linear-history-scans/ .linear-thread-root-scans/ .linear-thread-descendant-scans/ .linear-turn-marker-mismatches.json .linear-poll-health  server-timestamp cursors, persistent first-ingestion horizon, latest comment edit and history content heads, canonical comment roots, durable thread participation, issue snapshots, resumable provider scans, retained invariant failures, and poll health
...
2. **Bootstrap** - detect-only checks (tool/version problems, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   Home-local stale Herdr projection cleanup and the seven bootstrap MUTATING sweeps - non-executing legacy PR-check migration, fleet sync, secondmate convergence, secondmate liveness, pending remote handoff retry, Relay artifact writes, and Linear artifact writes - run only when this session actually holds the lock from step 1; the four network ones among them run in the deferred stage rather than in this section.
...
3. For `check:`, act on the named poll result, including merges, Linear events, Relay events, and process-to-event source results.
...
- `linear-operations` - load on every `linear:` `check:` wake and before Firstmate writes a Linear issue state, assignee, or comment.
